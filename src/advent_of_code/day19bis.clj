...
(defn- part-step [step]
  (let [[_ field op val to] (re-matches #"([xmas])([<>])(\d+):(\w+)" step)]
...
(defn- parse-part [part]
  (zipmap [:x :m :a :s] (u/parse-out-longs part)))
...
(defn- evaluate [part workflow]
  (loop [[[k op val to] & steps] workflow]
    (case k
      :last to
...
(defn- get-accepted [{:keys [workflows parts]}]
  (filter (partial accepted? workflows) parts))
